...
Building a K-surface from just these two conditions turns out to be supursingly difficult (but stay tuned for a future project). It turns out that a special parameterization (asymptotic coordinates) for the smooth version of a K-surface forms what is known as a Chebyshev net. For us this just means we are able to take a metric of the form 
...
To build a discrete Cheybshev net, we construct a tiling of rhombi on the Poincare disk. Using some differential geometry magic we then map this to a Chebyshev net on the sphere. We then can use even more dark geometry secrets to derive
