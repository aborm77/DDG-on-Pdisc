...
def _valid_segments(pts):
    """Split a (N, 3) array of 3D points into contiguous non-NaN segments."""
    segments = []
    current = []
    for p in pts:
...
class Surf_plot:
        ...
        if isinstance(surf_grid, mesh.Surf_create):
            ...
            if tiling_transforms is not None:
                ...
                for t, _ in tiling_transforms:
                    ...
                    copy.points = t(base_surf.mesh.points.copy())
                    copies.append(copy)
                full_mesh = pv.merge(copies).clean(tolerance=1e-10, absolute=True)
                self.surf = mesh.Surf_create.__new__(mesh.Surf_create)
                self.surf.base   = None
                self.surf.mesh   = full_mesh
                self.surf.edges  = full_mesh.extract_all_edges()
    ...
    def _fill_wireframe(self, surf_grid, transform, depth=0, rev=False, flip_colors=False):
        """Recursive helper: accumulate one tiling copy into self._uv_buf."""
        grid = surf_grid.grid
        c1, c2 = ('red', 'blue') if flip_colors else ('blue', 'red')
        ...
        for j in range(1, grid.shape[1]):
            for seg in _valid_segments(grid[:, j, :]):
                if transform is not None:
                    seg = transform(seg)
                n = len(seg)
                self._uv_buf[c1].append(pv.PolyData(seg, lines=[n] + list(range(n))))

        for i in range(1, grid.shape[0]):
            for seg in _valid_segments(grid[i, :, :]):
                if transform is not None:
                    seg = transform(seg)
                n = len(seg)
                self._uv_buf[c2].append(pv.PolyData(seg, lines=[n] + list(range(n))))

        if surf_grid.children is not None:
            for idx, child in enumerate(surf_grid.children):
                child_rev = (not rev) if idx % 2 != 0 else rev
                self._fill_wireframe(child, transform, depth=depth+1, rev=child_rev, flip_colors=flip_colors)

    def plot_wireframe(self, surf_grid, rev=False, transforms=None):
        """Plot u/v coordinate lines in red and blue, once per entry in transforms."""
        self._uv_buf = {'red': [], 'blue': []}
        for t, flip in (transforms if transforms is not None else [(None, False)]):
            self._fill_wireframe(surf_grid, t, rev=rev, flip_colors=flip)
        for color, bufs in self._uv_buf.items():
            if bufs:
                self.pl.add_mesh(pv.merge(bufs), line_width=2, color=color)
        del self._uv_buf
            

    def _fill_bds(self, surf_grid, transform, depth=0):
        """Recursive helper: accumulate one tiling copy of boundaries into self._bd_buf."""
        grid = surf_grid.grid

        for seg in _valid_segments(grid[0, :, :]):
            if transform is not None:
                seg = transform(seg)
            n = len(seg)
            self._bd_buf.append(pv.PolyData(seg, lines=[n] + list(range(n))))
        ...
        if surf_grid.children is not None:
            for child in surf_grid.children:
                self._fill_bds(child, transform, depth=depth+1)

    def plot_bds(self, surf_grid, transforms=None):
        """Plot sector boundary edges in black, once per entry in transforms."""
        self._bd_buf = []
        for t, _ in (transforms if transforms is not None else [(None, False)]):
            self._fill_bds(surf_grid, t)
        if self._bd_buf:
