...
class Surf_plot:
    ...
    """
    def __init__(self, surf_grid, plt_bps=True, plt_bds=True, plt_surf=False, plt_wireframe=True, plt_wilmore=False):
    ...
    def plot_wireframe(self, surf_grid, depth=0, rev=False):
        """Plot u- and v-coordinate lines on the surface in red and blue.

        Color convention mirrors Pdisc_plot.plot_lines: column slices (v-lines) in c1,
        row slices (u-lines) in c2, with colors swapped when rev=True. The middle child
        sector (index 1) flips rev because its u/v axes are exchanged relative to its parent.
        """
        if depth == 0:
        ...
        if surf_grid.children is not None:
            for idx, child in enumerate(surf_grid.children):
                ...
                self.plot_wireframe(child, depth=depth+1, rev=child_rev)
    ...
    def plot_bds(self, surf_grid, depth=0):
        """Plot sector boundary edges (u=0 and v=0 lines of each sector) in black."""
        if depth == 0:
            self._bd_buf = []
